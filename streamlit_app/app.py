...
# ── Sidebar — renders immediately before any Snowflake call ────────────────────
PAGES = [
    "🏠 Overview",
    "⚔️  Top Attackers",
    "🛡️  Top Defenders",
...
def get_conn_safe():
    """Returns (conn, error_string). Renders nothing — caller decides how to handle."""
    try:
...
# ── Connection status banner — shown on every page before any query ────────────
conn, conn_err = get_conn_safe()
if conn_err:
    st.error("Could not connect to Snowflake. Details below:")
    st.code(conn_err)
    st.stop()
...
if page == "🏠 Overview":
    ...
    with col_left:
        ...
        fig = px.bar(
            ...
            orientation="h", color="primary_type",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.spinner("Loading stats…"):
        df = query("""
            ...
            CROSS JOIN LATERAL (
                SELECT 'attack'           AS stat_name, f.attack           AS base_stat_value UNION ALL
                SELECT 'defense',                       f.defense                             UNION ALL
                SELECT 'hp',                            f.hp                                  UNION ALL
                SELECT 'speed',                         f.speed                               UNION ALL
                SELECT 'special-attack',                f.sp_attack                           UNION ALL
                SELECT 'special-defense',               f.sp_defense
            ) v
            WHERE d.primary_type IS NOT NULL
              AND v.base_stat_value IS NOT NULL
